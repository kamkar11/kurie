...
<head>
  ...
  <title>DOSTAWY.PL | ADMINISTRACJA</title>
...
<div class="login-box">
  ...
  <!-- /.login-logo -->
  <div class="container">
    <p class="login-box-msg"><b>Administrator Logowanie</b></p>
    ...
    <a href="login_kurier.php" class="text-center">Zaloguj się jako kurier</a>
<?php

    if(isset($_POST["submit1"] , $_POST["username"], $_POST["password"]))
    {
      $cur = oci_new_cursor($conn);
      $stmt = oci_parse($conn, "begin LOGIN_ADMIN(:PLOUG_CURSOR , :login , :haslo); end;");
      ...
      $adminLog = oci_fetch_array($cur, OCI_BOTH);


        $admin=0;

        $admin = oci_num_rows($stmt);
        if ($admin == 0) {
          ?>
                      <p>brak</p>
                ...
                } elseif ( ($adminLog['LOGIN'] == $_POST['username'] ) &&  ($adminLog['HASLO'] == $_POST['password'] )  ) {
